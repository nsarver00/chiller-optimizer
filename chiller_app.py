...
def print_all(best_group, best_kw, total_tons, daily_cost, monthly_cost, utilization):
    st.write("Building Load -->",round(tons,2),"tons")
...
if best_group is None:
	st.error("COOLING REQUIREMENT EXCEEDS CHILLER CAPACITY")
        ...
        total_tons,daily_cost,monthly_cost,utilization = calculate(best_group,best_kw,costperkwh,building_load_tons)
    ...
    #Creates results and dataframe for output csv file
    results = []  
    for chiller in best_group:
        ...
        results.append({"name":chiller["name"],
                        "eff":chiller["eff"],"tons":chiller["tons"],
                        "kw_usage":kw_chiller,
